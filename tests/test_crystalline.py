...
class TestSpectralAnalysis:
    ...
    def test_spectral_analysis_sine_wave(self):
        ...
        assert abs(peak_freq - f_signal) < 2
...
class TestFiltering:
    ...
    def test_spectral_filtering_lowpass(self):
        ...
        # Cutoff is normalized to Nyquist because the API has no fs argument.
        filtered = spectral_filtering(signal, cutoff=0.1, order=4, btype="low")

        assert len(filtered) == len(signal)
        assert np.all(np.isfinite(filtered))

    def test_spectral_filtering_bandpass(self):
        """Band-pass filtering should work with normalized cutoff edges."""
        signal = np.random.randn(1000)
        filtered = spectral_filtering(signal, cutoff=[0.02, 0.1], btype="band", order=4)
