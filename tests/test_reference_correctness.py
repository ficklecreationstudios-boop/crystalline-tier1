...
def test_linear_solve_supports_multiple_rhs():
    A = np.array([[3.0, 1.0], [2.0, 4.0]])
    b = np.array([[7.0, 1.0], [10.0, 2.0]])
    actual = backend.linear_algebra_solve(A, b)
    np.testing.assert_allclose(A @ actual, b)


def test_linear_solve_rejects_non_square_matrix():
    with np.testing.assert_raises(ValueError):
        backend.linear_algebra_solve(np.ones((2, 3)), np.ones(2))


def test_linear_solve_rejects_mismatched_rhs():
    with np.testing.assert_raises(ValueError):
        backend.linear_algebra_solve(np.eye(3), np.ones(2))


def test_linear_solve_rejects_singular_matrix():
    with np.testing.assert_raises(ValueError):
        backend.linear_algebra_solve(np.array([[1.0, 2.0], [2.0, 4.0]]), np.ones(2))
...
def test_matrix_multiply_rejects_non_matrices():
    with np.testing.assert_raises(ValueError):
        backend.matrix_multiply(np.ones(4), np.ones((4, 1)))
    with np.testing.assert_raises(ValueError):
        backend.matrix_multiply(np.ones((2, 2)), np.ones(2))


def test_matrix_multiply_rejects_incompatible_shapes():
    with np.testing.assert_raises(ValueError):
        backend.matrix_multiply(np.ones((2, 3)), np.ones((2, 2)))
